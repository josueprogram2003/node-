...
export const getPostId = async (req, res) => {
  try {
    ...
    const response = await pool.query("select * from fc_listar_post_id($1)", [
      id,
    ]);
    return res.status(200).json(response.rows);
  } catch (e) {
...
export const crearPost = async (req, res) => {
  try {
    ...
    return res.status(200).json({ message: "Post registrado correctamente" });
  } catch (e) {
    return res.status(500).json("Error al listar Post");
  }
};
...
export const updatePost = async (req, res) => {
  try {
    const id = parseInt(req.params.id);
    const { titulo, descripcion } = req.body;
    await pool.query("select fc_update_post($1,$2,$3)", [
      titulo,
      descripcion,
      id,
    ]);
